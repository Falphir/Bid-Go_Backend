...
namespace Bid_Go.Tests.Integration.Controllers
{
 ...
 {
 var company = new Company { Name = "Comp", CompanyName = "Comp Lda", Address = "A", Email = "c@x.com", Password = "x", PhoneNumber =900000000, NIF =111111111 };
 ...
 db.SaveChanges();
 var bid = new Bid { TransportRequestId = tr.TransportRequestId, DriverId = driver.Id, Value =80, DeliveryDeadline = DateTime.UtcNow.AddDays(2), Status = EBidStatus.Accepted };
 db.Bids.Add(bid);
 db.SaveChanges();
