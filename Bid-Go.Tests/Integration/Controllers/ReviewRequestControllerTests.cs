...
namespace Bid_Go.Tests.Integration.Controllers
{
 ...
 var fromDb = await db.Reviews.OfType<ReviewDriver>().FirstOrDefaultAsync(r => r.TransportRequestId == tr.TransportRequestId && r.DriverId == driver.Id);
 ...
 var (controller, db) = Build();
 var (company, driver, tr) = SeedCompletedRequest(db);
 db.Reviews.Add(new ReviewCompany { TransportRequestId = tr.TransportRequestId, CompanyId = company.Id, DriverId = driver.Id, Classification =4.5m, ServiceQuality =5, ClientSuport =4, TimeStamp = DateTime.UtcNow });
 db.Reviews.Add(new ReviewDriver { TransportRequestId = tr.TransportRequestId, CompanyId = company.Id, DriverId = driver.Id, Classification =4.0m, Punctuality =5, Behavior =4, TimeStamp = DateTime.UtcNow });
