...
namespace Bid_Go.Tests.Integration.Controllers
{
 ...
 {
 var company = new Company { Name = "C", CompanyName = "CC", Address = "A", Email = $"c{companyId}@c.com", Password = "x", PhoneNumber =900000000+companyId, NIF =111111111+companyId };
 ...
 var tr = SeedTransportRequest(db);
 var bid = new Bid { DriverId =11, TransportRequestId = tr.TransportRequestId, Value =60, DeliveryDeadline = DateTime.UtcNow.AddDays(2), Status = EBidStatus.Pendent };
 db.Bids.Add(bid);
 await db.SaveChangesAsync();

 var dto = new BidUpdateDTO { Value =40, DeliveryDeadline = DateTime.UtcNow.AddDays(1) };
 ...
 var tr = SeedTransportRequest(db);
 var bid = new Bid { DriverId =12, TransportRequestId = tr.TransportRequestId, Value =60, DeliveryDeadline = DateTime.UtcNow.AddDays(2), Status = EBidStatus.Pendent };
 db.Bids.Add(bid);
 await db.SaveChangesAsync();
 ...
 var tr = SeedTransportRequest(db);
 var bid = new Bid { DriverId =13, TransportRequestId = tr.TransportRequestId, Value =60, DeliveryDeadline = DateTime.UtcNow.AddDays(2), Status = EBidStatus.Pendent };
 db.Bids.Add(bid);
 await db.SaveChangesAsync();
 ...
 var tr = SeedTransportRequest(db);
 db.Bids.AddRange(
 new Bid { DriverId =14, TransportRequestId = tr.TransportRequestId, Value =60, DeliveryDeadline = DateTime.UtcNow.AddDays(2), Status = EBidStatus.Pendent },
 new Bid { DriverId =15, TransportRequestId = tr.TransportRequestId, Value =40, DeliveryDeadline = DateTime.UtcNow.AddDays(2), Status = EBidStatus.Pendent }
 );
 await db.SaveChangesAsync();
 ...
 var tr = SeedTransportRequest(db);
 db.Drivers.Add(new Driver { Name = "D16", Email = "d16@x.com", Password = "x", PhoneNumber =911116116, NIF =123456716 });
 await db.SaveChangesAsync();
 var d = await db.Drivers.FirstAsync();
 db.Bids.AddRange(
 new Bid { DriverId = d.Id, TransportRequestId = tr.TransportRequestId, Value =80, DeliveryDeadline = DateTime.UtcNow.AddDays(2), Status = EBidStatus.Pendent },
 new Bid { DriverId = d.Id, TransportRequestId = tr.TransportRequestId, Value =50, DeliveryDeadline = DateTime.UtcNow.AddDays(1), Status = EBidStatus.Pendent }
 );
 await db.SaveChangesAsync();
 var result = await controller.GetActiveBids(tr.TransportRequestId, orderBy: "value", descending: false);
 var ok = Assert.IsType<OkObjectResult>(result);
 var anonList = Assert.IsType<List<object>>(ok.Value);
