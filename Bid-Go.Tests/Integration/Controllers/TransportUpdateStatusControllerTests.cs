...
namespace Bid_Go.Tests.Integration.Controllers
{
 ...
 var service = new TransportUpdateStatusService(repo, notifications, logger);
 var controller = new TransportUpdateStatusController(service);
 ...
 {
 var company = new Company { Name = "C", CompanyName = "CC", Address = "A", Email = "c@x.com", Password = "x", PhoneNumber =900000000, NIF =111111111 };
 var driver = new Driver { Name = "D", Email = "d@x.com", Password = "x", PhoneNumber =911111111, NIF =222222222 };
 ...
 db.SaveChanges();
 var p1 = new Bid { TransportRequestId = tr.TransportRequestId, DriverId = driver.Id, Status = EBidStatus.Pendent, Value =50, DeliveryDeadline = DateTime.UtcNow.AddDays(4) };
 var p2 = new Bid { TransportRequestId = tr.TransportRequestId, DriverId = driver.Id, Status = EBidStatus.Pendent, Value =60, DeliveryDeadline = DateTime.UtcNow.AddDays(4) };
 ...
 var result = await controller.UpdateRequestStatus(tr.TransportRequestId, userID: company.Id, dto);
 ...
 var result = await controller.UpdateRequestStatus(tr.TransportRequestId, userID: driver.Id, dto);
 var ok = Assert.IsType<OkObjectResult>(result);
 var fromDb = await db.TransportRequests.FindAsync(tr.TransportRequestId);
